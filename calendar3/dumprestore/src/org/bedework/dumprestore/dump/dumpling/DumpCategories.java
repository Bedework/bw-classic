 ...
 * @version 1.0
 */
public class DumpCategories extends Dumpling {
  /** Constructor
   *
   * @param globals
   */
  public DumpCategories(DumpGlobals globals) {
    super(globals);
  }
  ...
  public void dumpSection(Iterator it) throws Throwable {
    ...
    tagEnd(sectionCategories);
  }

  private void dumpCategory(BwCategory val) throws Throwable {
    tagStart(objectCategory);

    shareableEntityTags(val);

    taggedVal("word", val.getWord());
    taggedVal("description", val.getDescription());

    tagEnd(objectCategory);
