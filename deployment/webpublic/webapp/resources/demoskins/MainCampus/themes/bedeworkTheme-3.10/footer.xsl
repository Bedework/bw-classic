    ...
    <xsl:if test="$showFootForm = 'true'">
      <div id="footForms">
        <form name="skinSelectForm" method="post" action="{$setup}">
          ...
          <select name="skinPicker" onchange="window.location = this.value" title="skin picker">
            ...
            <option value="{$setup}&amp;skinNameSticky=bedeworkTheme-3.9">
              <xsl:copy-of select="$bwStr-Foot-BwTheme3p9" />
            </option>
            <option value="{$setup}&amp;skinNameSticky=default">
